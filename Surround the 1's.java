gfg link: https://practice.geeksforgeeks.org/problems/surround-the-1s2505/1
...
class Solution{
    ...
    public int  Count(int[][] matrix){
        ...
        for(int i=0;i<row;i++){
            for(int j=0;j<col;j++){
                if(matrix[i][j]==1){
                    if(isValid(i-1,j)==true && matrix[i-1][j]==0)
                    countOfZeros++;
                    if(isValid(i+1,j)==true && matrix[i+1][j]==0)
                    countOfZeros++;
                    if(isValid(i,j-1)==true && matrix[i][j-1]==0)
                    countOfZeros++;
                    if(isValid(i,j+1)==true && matrix[i][j+1]==0)
                    countOfZeros++;
                    if(isValid(i-1,j-1)==true && matrix[i-1][j-1]==0)
                    countOfZeros++;
                    if(isValid(i-1,j+1)==true && matrix[i-1][j+1]==0)
                    countOfZeros++;
                    if(isValid(i+1,j-1)==true && matrix[i+1][j-1]==0)
                    ...
                    if(countOfZeros%2==0 && countOfZeros!=0)
                    count++;
