gfg link: https://practice.geeksforgeeks.org/problems/prefix-match-with-other-strings/1

QUES:
Given an array of strings arr[] of size n, a string str and an integer k. The task is to find the count of strings in arr[] whose prefix 
...
Your Task:  
You don't need to read input or print anything. Your task is to complete the function klengthpref() which takes the array of strings 
arr[], its size n and an integer k, a string str as input parameters and returns the count of strings in arr[] whose prefix of length k 
...
class Solution{
    public int klengthpref(String[] arr, int n, int k, String str){
        ...
        String prefix="";
        if(str.length()>=k){
            prefix=str.substring(0,k);
        }
        else return 0;
        int count=0;
        for(int i=0;i<n;i++){
            if(arr[i].length()>=k){
                 String check=arr[i].substring(0,k);
