gfg link => https://practice.geeksforgeeks.org/problems/fill-up-buckets3500/1

//QUES: Given n buckets and infinite number of balls. The maximum capacity of each bucket is given in an array capacity[]. Find the number of ways to fill the buckets with balls such that each bucket has atleast 1 ball and all the buckets have distinct number of balls in them.
...
//CODE:
class Solution{
    public int totalWays(int n, int[] a) {
        // code here
        Arrays.sort(a);
        long ans=1;
        int mod = 1000000007;
